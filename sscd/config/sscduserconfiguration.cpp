...
SSCDUserConfiguration::SSCDUserConfiguration(QWidget* parent, const QVariantList& args)
					: KCModule(KGlobal::mainComponent(), parent)
{
	...
	//addConfig(SSCDConfiguration::self(), this);
...
void SSCDUserConfiguration::slotChanged()
{
	emit changed(true);
}

void SSCDUserConfiguration::activateUserConfiguration(bool active)
{
...
void SSCDUserConfiguration::initDb()
{
	if (!db->init())
	{
...
void SSCDUserConfiguration::addUser()
{
...
void SSCDUserConfiguration::deleteUser()
{
	...
	QString modelDir = KStandardDirs::locateLocal("data", "sscd/models/")+username+"/";
...
bool SSCDUserConfiguration::recursiveDelete(const QString& dirPath)
{
...
QByteArray SSCDUserConfiguration::encryptPassword(const QString& password)
{
	return QCryptographicHash::hash(password.toUtf8(),QCryptographicHash::Sha1);
}


void SSCDUserConfiguration::changePassword()
{
	if (!ui.tvUser->currentIndex().isValid())
		return;
...
void SSCDUserConfiguration::load()
{
	initDb();
	KCModule::load();
}

void SSCDUserConfiguration::save()
{
	KCModule::save();
}


SSCDUserConfiguration::~SSCDUserConfiguration()
{	
	db->deleteLater();
}
