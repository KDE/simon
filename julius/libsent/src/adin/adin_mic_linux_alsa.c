...
adin_alsa_end()
{
  ...
  snd_pcm_t *close_handle;	///< Audio handler
  close_handle=handle;
  handle=NULL;

  boolean worked=TRUE;
  if (snd_pcm_state(close_handle) == SND_PCM_STATE_RUNNING)
  {
    if ((ret = snd_pcm_drop(close_handle)) != 0) {
      worked = FALSE;
      fprintf(stderr, "Could not drop frames: %d", ret);
    }
  }
  if ((ret = snd_pcm_close(close_handle)) != 0) {
    fprintf(stderr, "Could not close handle: %d", ret);
    worked = FALSE;
  }
  if ((ret=snd_pcm_hw_free(close_handle)) != 0) {
