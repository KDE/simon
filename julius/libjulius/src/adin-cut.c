...
adin_cut(int (*ad_process)(SP16 *, int, Recog *), int (*ad_check)(Recog *), Recog **recog_p)
{
  ...
  for (;*a_ptr != NULL;) {
	...
	cnt = (*(a->ad_read))(&(a->buffer[a->bp]), a->bpmax - a->bp);
      ...
      //fprintf(stderr, "Count: %d, Recog: %d\n", cnt, *recog_p);
    ...
    */
    ...
    if ((a->end_of_stream == TRUE) && a->bp == 0) break;
  }
...
break_input:
  ...
  /* pause input */
  /****************/
  if (a->end_of_stream == TRUE) {			/* input already ends */
    /* execute callback */
    callback_exec(CALLBACK_EVENT_SPEECH_STOP, recog);
 ...
 */
static void
adin_thread_input_main(Recog *recog)
{
  int ret;
