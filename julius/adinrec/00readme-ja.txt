...
OPTIONS
   ...
   Concerning Julius options
        -input  {mic|rawfile|adinnet|stdin|netaudio|esd|alsa|oss}
          ...
          netaudio はDatLinkのサーバから， stdinは標準入力から音声入力を行う．
        ...
        -48
          48kHzで入力を行い，16kHzにダウンサンプリングする． これは 16kHz のモ
          デルを使用しているときのみ有効である． ダウンダンプリングの内部機能
          は sptk から 移植された． (Rev. 4.0)
        ...
        -adport  port_number
          ...
          -input adinnet 使用時，接続を受け付ける adinnet のボート番号を指定す
          る．(default: 5530)

        -nostrip
          音声取り込み時，デバイスやファイルによっては，音声波形中に振幅 が
          "0" となるフレームが存在することがある．Julius は通常，音声 入力に含
          まれるそのようなフレームを除去する．この零サンプル除去が うまく動か
          ない場合，このオプションを指定することで自動消去を 無効化することが
          できる．
