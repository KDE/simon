...
{
	if (!rec->finish()) 
		QMessageBox::critical(this, "Aufnehmen fehlgeschlagen", QString("Abschlie�en der Aufnahme fehlgeschlagen. M�glicherweise ist die Aufnahme fehlerhaft.\n\nTip: �berpr�fen Sie ob Sie die n�tigen Berechtigungen besitzen um auf %1 schreiben zu d�rfen!").arg(this->filename));
...
void RecWidget::deleteSample()
{
	if(QFile::remove(this->filename))
	{
		...
		emit sampleDeleted();
	} else QMessageBox::critical(this, "Fehler beim L�schen", 
...
void RecWidget::setupWidgets()
{
	...
	pbPlay = new QPushButton(QIcon(":/images/icons/media-playback-start.svg"), "Abspielen",
				 this);
	pbDelete = new QPushButton(QIcon( ":/images/icons/emblem-unreadable.svg"), "L�schen",
				   this);
