...
bool IniReader::load(QString path)
{
	...
	if (path.isEmpty()) {
		Logger::log(QObject::tr("[ERR] Kein Dateiname angegeben - konnte INI Datei nicht lesen"));
		return false;
	}
	
	QFile doc(path);
	if (!doc.open(QIODevice::ReadOnly|QIODevice::Text)) {
		Logger::log(QObject::tr("[ERR] Konnte INI-Datei %1 nicht zum lesen öffnen").arg(path));
		return false;
	}
	...
	while (!ts.atEnd())
	{
		...
		if (pair.count() == 2) 
// 			Logger::log(QObject::tr("[INF] Ignoriere Zeile %1 in Datei %2: \"%3\"").arg(lineC).arg(path).arg(line));
