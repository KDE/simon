...
private:
	...
	char charKey;
public:
	Shortcut(QString name,
		int modifiers, int actionKeys, int functionKeys, int movementKeys, char charKey)
	{
		this->name = name;
	...
	bool operator==(Shortcut s2) {
		return ((s2.getName() == name) && (modifiers==s2.getModifiers()) && 
