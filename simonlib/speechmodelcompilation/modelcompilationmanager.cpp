...
bool ModelCompilationManager::increaseMixtures()
{
  ...
  if (!buildHMM17()) {
    analyseError(i18n("Could not generate HMM17. Please check the paths to HERest (%1), the config (%2) and to the stats-file (%3).", hERest, KStandardDirs::locate("data", "simon/scripts/config"), tempDir+"/stats"));
    return false;
  }
  ...
  if (!buildHMM20()) {
    analyseError(i18n("Could not generate HMM20. Please check the paths to HERest (%1), the config (%2) and to the stats-file (%3).", hERest, KStandardDirs::locate("data", "simon/scripts/config"), tempDir+"/stats"));
    return false;
  }
  ...
  if (!buildHMM23()) {
    analyseError(i18n("Could not generate HMM23. Please check the paths to HERest (%1), the config (%2) and to the stats-file (%3).", hERest, KStandardDirs::locate("data", "simon/scripts/config"), tempDir+"/stats"));
    return false;
  }
